...
> 本包为独立自研实现（非 fork）。配置约定与 [dsh-feishu-connect](https://github.com/limingboGitHub/dsh-feishu-connect)（MIT）保持一致（`~/.cc-connect/feishu.config.json`），因此从该插件迁移**零配置改动**；两者不要同时安装（同一飞书 App 的 WS 长连接互踢）。
...
- **每聊天独立会话**：每个飞书聊天拥有专属 Agent 会话池（绝不串进 GUI 会话）；首条消息自动创建；会话持久化（重启恢复）；`/new [名称]`、`/switch <序号>`、`/list`、`/help`
- **处理中表情**：消息到达加 `OnIt` 表情，回复送达后撤销（可配 `reactionEmoji`，`none` 关闭）
- **工具**：`feishu_send`（agent 可主动发消息，`appId` 指定机器人，缺省发到最近会话）
- **保活**：helper 子进程崩溃自动重启 + 凭据变更自动重连 + 官方 SDK 自带重连 + 连接状态可观测（日志/状态接口）
...
配置写在 **`~/.cc-connect/feishu.config.json`**（与仓库解耦，全局约定）：
...
会话状态持久化在 `~/.cc-connect/state-<appId>.json`。配置支持热更新（10 秒轮询），改完无需重启。
...
- 配置热读：10s 检查变更；helper 每机器人一个子进程，崩溃自动重启
